...
const hasFullOverlap = ({a, b}) => isRangeAContainedInRangeB(a, b) || isRangeAContainedInRangeB(b, a);

const hasAnyOverlap = ({a, b}) => doesRangeAOverlapWithRangeB(a, b) || doesRangeAOverlapWithRangeB(b, a);

const parseRange = R.pipe(
  R.split("-"),
  R.map(Number),
  ([a, b]) => ({ min: a, max: b }),
)
